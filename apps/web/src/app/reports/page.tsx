...
import { getGoals, getLatestImportStatus, getReport } from "@/lib/dashboard/data";
...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ range?: string; saved?: string; error?: string }> }) {
  ...
  const [result, scoreReportResult, goalsResult, importStatus] = await Promise.all([reportPromise, scoreReportPromise, getGoals(), getLatestImportStatus()]);
  ...
  return <DashboardShell active="/reports" eyebrow="Reports" title="Look closer when you want to" description="Switch between short and longer windows to review sleep, activity, and cardio sections with the same timezone and coverage rules as the dashboard."><div className="space-y-6"><ImportStatusNotice importData={importStatus.status === "ok" ? importStatus.data : null} /><RangeTabs range={range} />{params.saved === "snapshot" ? <p className="rounded-2xl border border-emerald-200/20 bg-emerald-300/10 p-4 text-sm text-emerald-100" role="status">This score version was saved as an owner-only snapshot.</p> : null}{params.error === "snapshot-save" ? <p className="rounded-2xl border border-amber-200/20 bg-amber-300/10 p-4 text-sm text-amber-100" role="alert">The score snapshot could not be saved. The report itself is still available.</p> : null}<WellnessScoreView score={score} />{result.status === "error" ? <SafeErrorState /> : <ReportsView report={result.data} />}</div></DashboardShell>;
}

function parseRange(value: string | undefined): ReportRange {
  if (value === "latest") return "latest";
  const parsed = Number(value);
  return REPORT_RANGES.includes(parsed as (typeof REPORT_RANGES)[number]) ? parsed as (typeof REPORT_RANGES)[number] : 28;
}

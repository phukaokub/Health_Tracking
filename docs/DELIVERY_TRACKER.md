...
- Current gate: Step 5 implementation is merged in PR #62; the private actual-workbook staging gate is verified, while the follow-up parser guard and Huawei sheet-cap fix remain uncommitted for the next review slice. Step 3's hosted synthetic two-user, quota/outage, and cleanup suite remains explicitly deferred and is out of scope.
- Current branch: `main`.
...
| 5 | Legacy XLS allowlisted backfill and precedence | Staging verification complete; follow-up fix pending review | Local/CI parser, schema, API, UI, and browser evidence are green; private actual Huawei BIFF8 `.xls` staging run completed with inserted/conflicted/excluded counts, a redacted webhook log, and cleanup evidence |

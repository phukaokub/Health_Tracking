...
| Integration | Purpose and data | Environments | Status on 2026-07-30 | Credential/configuration owner | Verification and failure behavior |
| --- | --- | --- | --- | --- | --- |
...
| Hosted Supabase staging | Non-production Auth, Postgres, private Storage | Preview/staging | Active shared staging: `Health_Tracking` (`gdccossstmochzfgjqxz`), `ap-southeast-1`; Step 4 migrations and worker policies applied | Supabase organization/project owner | Dedicated Auth worker, active-lease Storage read, typed persistence, 24-hour recovery cleanup, owner purge, explicit grants, and RLS were verified with generated data. Step 3's broader hosted two-user/quota/outage suite remains deferred; no provider evidence is production evidence |
...
| Vercel web project | Builds and serves Next.js | Preview/staging/production | Active staging project `health-tracking-web-staging`; its Vercel Production target is used only as the staging project runtime | Vercel team/project owner | Public staging web/Supabase/API values are configured in the target that serves the staging alias; hosted Auth and upload-to-queue observed |
| Vercel API project | Builds and serves Go API | Preview/staging/production | Active separate staging project `health-tracking-api-staging`; its Vercel Production target serves only the stable staging alias | Vercel team/project owner | Health 200, unauthenticated owner routes 401, exact-origin CORS 204, encrypted worker variables on Preview/stable staging, and default-off runtime gate verified; no application-production project was changed |
| Background parser runtime | Executes bounded Step 4 normalization jobs with a dedicated short-lived Auth identity | Staging/production | Validated as a manual, default-off Vercel Function in staging; production deferred | Runtime, Supabase, and operations owners | Generated 72 MiB and five-file 330 MiB benchmarks met time/memory headroom; a real private-Storage synthetic job persisted six canonical records, replayed idly, honored the 24-hour recovery rule, cleaned its source, and left zero residual test data after owner/account deletion |
...
| --- | --- | --- | --- |
| INT-001 | Confirm `Health_Tracking` as shared staging or create an isolated replacement; then decide whether later PRs need Supabase Branching | Before remote migration/Storage verification in Step 3 | Accepted by user for shared synthetic staging; reassess Branching before parallel PR work |
| INT-002 | Vercel custom Staging environment versus branch-specific Preview fallback | Before first stable hosted release candidate | Branch-specific Preview accepted for this gate; exact projects and URLs recorded in the hosted PR |
...
| INT-007 | Go foreground/background Supabase access model and least-privileged worker credential | Before Step 4 real worker execution | Foreground accepted in ADR 0002; ADR 0005 Option A is implemented and verified for the manual staging worker. Production remains a separate Step 9 decision |

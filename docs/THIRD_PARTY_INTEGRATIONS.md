...
| Hosted Supabase staging | Non-production Auth, Postgres, private Storage | Preview/staging | Active shared synthetic staging: `Health_Tracking` (`gdccossstmochzfgjqxz`), `ap-southeast-1`; canonical migrations applied | Supabase organization/project owner | `rls_auto_enable()` public execution revoked; all application tables RLS-enabled; preview API/web health green. Authenticated synthetic smoke is pending provider signup cooldown (HTTP 429); no personal data |
...
| Vercel web project | Builds and serves Next.js | Preview/staging/production | Active preview project `health-tracking-web-staging` | Vercel team/project owner | Preview build and page HTTP 200; Supabase/API values scoped to Preview; no production configuration |
| Vercel API project | Builds and serves Go API | Preview/staging/production | Active preview project `health-tracking-api-staging` | Vercel team/project owner | Preview health HTTP 200; unauthenticated routes return 401; Supabase/API values scoped to Preview |

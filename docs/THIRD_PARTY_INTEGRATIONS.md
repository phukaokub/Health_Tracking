...
| Hosted Supabase staging | Non-production Auth, Postgres, private Storage | Preview/staging | Candidate discovered, not accepted/configured: `Health_Tracking` (`gdccossstmochzfgjqxz`), `ap-southeast-1` | Supabase organization/project owner; confirmation pending | Empty repository migration history and two pre-existing `rls_auto_enable` security warnings block adoption until reviewed; previews fail closed |
...
| Vercel web project | Builds and serves Next.js | Preview/staging/production | Planned; provider audit found no existing project | Vercel team/project owner | Build, page/Auth smoke, env audit, deployment diagnostics; rollback to known-good deployment |
| Vercel API project | Builds and serves Go API | Preview/staging/production | Planned; provider audit found no existing project | Vercel team/project owner | Health/JWKS/authenticated route smoke; web must show a safe unavailable state on outage |
...
| --- | --- | --- | --- |
| INT-001 | Confirm `Health_Tracking` as shared staging or create an isolated replacement; then decide whether later PRs need Supabase Branching | Before remote migration/Storage verification in Step 3 | No mutation; active candidate remains untrusted until ownership and existing database code are reviewed |

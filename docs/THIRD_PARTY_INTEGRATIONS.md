...
| Hosted Supabase staging | Non-production Auth, Postgres, private Storage | Preview/staging | Active shared staging: `Health_Tracking` (`gdccossstmochzfgjqxz`), `ap-southeast-1`; canonical migrations applied | Supabase organization/project owner | `rls_auto_enable()` public execution revoked; all application tables RLS-enabled; user-verified Google Auth and authenticated upload-to-queue. Hosted synthetic two-user/quota/outage/cleanup smoke is deferred; no provider evidence is production evidence |
...
| Vercel API project | Builds and serves Go API | Preview/staging/production | Active staging project `health-tracking-api-staging` | Vercel team/project owner | Health HTTP 200; unauthenticated routes return 401. API values require the serving target before hosted authenticated API smoke |
| Background parser runtime | Executes bounded Step 4 normalization jobs; current slice authenticates a dedicated worker and runs a synthetic parser/recovery benchmark only | Staging/production | Staging synthetic trigger implemented on Vercel Preview; real Storage/persistence execution planned | Runtime, Supabase, and operations owners | Trigger secret and worker claim are server-only; synthetic benchmark must pass before any real-import adapter is enabled; no production runtime |
...
| INT-007 | Go foreground/background Supabase access model and least-privileged worker credential | Before Step 4 real worker execution | Foreground accepted in ADR 0002; ADR 0005 Option A accepted for staging synthetic trigger; Storage/persistence adapter still requires separate proof |
